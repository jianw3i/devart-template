...
function SimulationRenderer(WIDTH, renderer) {
	...
	var encodeShader = new THREE.ShaderMaterial( {
		...
		vertexShader: document.getElementById( 'vertexShader' ).textContent,
		fragmentShader: document.getElementById( 'encodeFloatShader' ).textContent

	} );

	// var encodedFloats = new THREE.DataTexture( new Float32Array( PARTICLES * 4 * 4), WIDTH, WIDTH, THREE.RGBAFormat, THREE.FloatType );
	// encodedFloats.minFilter = THREE.NearestFilter;
	// encodedFloats.magFilter = THREE.NearestFilter;
	// encodedFloats.needsUpdate = true;
	// encodedFloats.flipY = false;

	var encodedFloats = new THREE.WebGLRenderTarget(WIDTH * 4, WIDTH * 4, {
	...
	pixels = new Uint8Array(WIDTH * WIDTH * 4 * 4);
	// var 
	pixels32 = new Float32Array(WIDTH * WIDTH * 4);
	...
	this.renderRenderAs = function (shader, input, output) {
	...
	this.simulate = function( delta ) {
		...
		encodeShader.uniforms.texture.value = rtPosition2;
