...
function SimulationRenderer(WIDTH, renderer) {
	...
	var encodedFloats = new THREE.WebGLRenderTarget(WIDTH * 2, WIDTH * 2, {
	...
	encodedFloats.generateMipmaps = false;

	pixels = new Uint8Array(WIDTH * WIDTH * 4 * 4);
	pixels32 = new Float32Array(pixels.buffer);
	...
	function generateColorTexture() {
		...
		for ( var k = 0; k < PARTICLES * 4; k += 4 ) {
			...
			a[ k + 1 ] = c.g;
			a[ k + 2 ] = c.b;
			a[ k + 3 ] = 1; // 1 to enable, 0 to disabled
